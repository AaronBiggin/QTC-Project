...
def get_rx_one_mapping(graph_max, G): #get_rx_one_mapping(tmp_graph, coupling_graph)
    ...
    #returns one possible correct mapping of the parition on the coupling graph
    item = next(vf2)
    #first being big graph(architecture graph) and second being subgraph(parition)
    reverse_mapping = {rx_nx_s[value]: rx_nx_G[key] for  key, value in item.items()}
    #key is a and value logical qubit [key] to physical space[value]
    return reverse_mapping #return is nx
...
#paritions including one or more layers.
#can be executed on the same layer of physical qubits as logical qubits
...
def complete_mapping(i, embeddings, indices, coupling_graph):
    #embeddings--> key = logical qubit and value = physical space
    cur_map = embeddings[i]
    unoccupied = [value for value in coupling_graph.nodes() if value not in cur_map]
    #Processes each logical qubit (index) that is currently unassigned (-1).
    for index in indices:
        ...
        if i != 0:  #If pre_map is not empty
         ...
         # If there is a previous embedding (i != 0) and the logical qubit was assigned a physical qubit in the previous mapping that is still unoccupied, reuse that assignment.
         #If a value from unoccupied which is the logical qubit is equalt to the embeddings logical qubit that also has a physical qubit reuse that mapping
        ...
        if i != len(embeddings) - 1 and flag == False:
            for j in range(i+1, len(embeddings)):
                if embeddings[j][index] != -1 and embeddings[j][index] in unoccupied:
                    ...
                    flag = True
                    break
       #If the logical qubit is assigned in a future mapping and that assignment is available in the current context, use it.
        ...
        if flag == False:
            if i != 0:
                ...
                for node in unoccupied:
                    #pick unoccupied physical qubit
                ...
                #find which unoccupied physical qubit is closest to embeddings[i-1][index](ie find the closest physical qubit that is not )
                cur_map[index] = min_node
                unoccupied.remove(min_node)
                #Occupy the logical physical qubit with that new qubit found
                flag = True
        #If the previous if statments did not provide a solution. If no suitable assignment is found, pick the
        # unoccupied physical qubit that is closest (based on the shortest path) to the last assignment of the logical qubit.
                ...
                for j in range(i+1, len(embeddings)):
                    if embeddings[j][index] != -1:
                        ...
                        flag = True
                        break
#Assign random logical qubit to physical qubit.
...
def get_embeddings(partition_gates, coupling_graph, num_q, arch_size, Rb, initial_mapping=None):
    ...
    for i in range(begin_index, len(partition_gates)): 
        ...
        #If the gates of that parititon can be arranged onto the coupling graph. Extend the range of the coupling graph 
        if not rx_is_subgraph_iso(coupling_graph, tmp_graph):
            coupling_graph = extend_graph(coupling_graph, arch_size, Rb) #extend architecture size and find the coupling graph that can be used
            extend_position.append(i)
        #What happens if the temporary graph when it is extended is not embeddable on the coupling graph(physical qubits)
        next_embedding = get_rx_one_mapping(tmp_graph, coupling_graph)
        #returns one possible correct mapping of the parition on the coupling graph
        next_embedding = map2list(next_embedding,num_q)
        #The keys which represent the logical qubits. The value in the array reprseents the physical .
        #next_embedding[0] = physical_location of the qubit
        embeddings.append(next_embedding)
        
        
        #for each parition there is should be an embedding from each gate to physical spaces(coupling graph)

    for i in range(begin_index, len(embeddings)):
        indices = [index for index, value in enumerate(embeddings[i]) if value == -1]
        #checks to see if there is a logical qubit that is not in physical space (-1), add the list of logical qubits that have no physical mapping
        #indicies is list of logical qubits in physical space
        if indices: #if indicies is not empty
            embeddings[i] = complete_mapping(i, embeddings, indices, coupling_graph)
            #basically it finds a logical qubit to map to a physical qubit-> first it tries to recycle previous and future qubit mappings, then it finds the closes qubit mappings to the preivous mappings and finally finds a random qubit mapping.
            #extendposition is a list of indexes corresponding to paritions that required the coupling graph to be extended
